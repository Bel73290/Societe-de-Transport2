...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_search'])) {
    ...
    if (empty($colis)) {
        ...
        if (!$result) {
            ...
            echo "Colis trouvé ! Statut : " . htmlspecialchars($row['statut']) .
                 ", Date de réception prévue : " . htmlspecialchars($row['date_reception']);
        } else {
            echo "Le colis n'existe pas dans la base de données.";
        }
    }
    exit(); // Empêche le reste du HTML d'être renvoyé
}
